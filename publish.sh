...
# Verify files exist
if [ ! -f "llm-stream-processor.js" ]; then
  echo -e "${RED}Error: llm-stream-processor.js not found${NC}"
  exit 1
fi

if [ ! -f "llm-stream-processor.d.ts" ]; then
  echo -e "${RED}Error: llm-stream-processor.d.ts not found${NC}"
  exit 1
fi
...
echo -e "${YELLOW}- Files to publish: llm-stream-processor.js, llm-stream-processor.d.ts, README.md, LICENSE${NC}"
echo ""
